...
def soft_fscore(soft_recall, soft_precision):
    if (soft_recall == 0.0) or (soft_precision == 0.0):
        return 0.0
    else:
...
parser.add_argument(
    "--prediction", required=True, type=argparse.FileType("r"), help=("File containing the predicted knowledge graph.")
)
parser.add_argument("--significant", type=int, default=1, help="significant digits to output (default: 2)")
parser.add_argument(
    "--alignment", action="store_true", help="whether to print out the alignment between prediction and reference"
)
...
metrics = {
    ...
    "soft match": {"precision": sp, "recall": sr, "fscore": soft_fscore(sp, sr)},
}
